...
int main()
{
	...
	std::vector<CoinMoney>::iterator it;
	int compareValue = 2400;
	it = std::find_if(coinVec.begin(), coinVec.end(), [compareValue = compareValue](const CoinMoney& m){
		return compareValue < m.get_value();
	});

	if (it != coinVec.end())
		cout << compareValue << "원보다 큰 첫 동전 다발은 " << *it;
	else
		cout << compareValue << "원보다 큰 첫 동전 다발은 없음\n";
