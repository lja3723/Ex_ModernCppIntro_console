...
int main()
{
	...
	functor.compareValue = 2400;
	it = std::find_if(coinVec.begin(), coinVec.end(), functor);

	if (it != coinVec.end())
		cout << functor.compareValue << "원보다 큰 첫 동전 다발은 " << *it;
	else
		cout << functor.compareValue << "원보다 큰 첫 동전 다발은 없음\n";
